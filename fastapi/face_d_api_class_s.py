...
class AnimeFace_det:
    ...
    def face_det_head(self, img_data, ratio, shift, confidence_level):
        ...
        try:
            ...
            for i in range(dnum):
                box=predict_bbox[i]
                print("bbox"+str(i)+"=",box,box[0],box[1],box[2],box[3])
                box_disp=(box[0],box[1]),(box[2],box[3])
                print("box_disp=",box_disp)

                image = cv2.rectangle(img_data,(int(box[0]),int(box[1])),(int(box[2]),int(box[3])),(0,0,255), thickness=3)
